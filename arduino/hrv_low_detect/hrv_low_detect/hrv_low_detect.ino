...
void loop() {
  ...
  if (lastBinOut == 0 && binOut == 1) {
    lastBeat = beat;
    beat = millis();
    int interval = beat - lastBeat;
    intervals[counter++%INTERVAL_LEN] = interval;
    for (int i = INTERVAL_LEN - 1; i >= 0; i--) 
        Serial.print((String)intervals[i] + " ");
    Serial.println();
    Serial.println("==");
    Serial.println();
    delay(50);
